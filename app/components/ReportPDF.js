...
const ReportPDF = ({ report, createdDate, createdTime }) => {
    ...
    return (
        <div className=" w-[350px] h-full  md:h-[600px] pb-6 max-h-full sm:w-[700px] lg:w-[850px] bg-white flex flex-col items-center rounded-md shadow-sm">
            ...
            <main className="mt-2 p-2 md:p-0 flex flex-col items-center md:flex-row md:items-start w-full justify-around">
                ...
                <div className="flex flex-col md:w-[40%]">
                    <img src={`${process.env.NEXT_PUBLIC_IMAGE_URL}${report.image}`} className=" w-[200px] max-h-[250px] md:w-[350px] max-w-[370px] md:max-h-[400px]" alt="report img" />
                    <h1 className="capitalize text-start text-sm md:text-xl font-medium text-slate-800">Upscaled {report.domain} image</h1>
                </div>

                <div className="md:w-1/2">
                    <h2 className=" text-center mt-2 md:mt-0  md:text-start font-medium text-lg md:text-3xl text-slate-700 ">Diagnoses by SuperRay</h2>
